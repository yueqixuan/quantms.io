...
def convert_msstats_differential(
    ...
    Args:
        ...
        output_prefix_file: Optional prefix for output files
...
@click.option("--msstats-file", help="MSstats differential file", required=True, type=click.Path(exists=True, dir_okay=False, path_type=Path))
...
@click.option("--project-file", help="quantms.io project file", type=click.Path(exists=True, dir_okay=False, path_type=Path))
@click.option("--protein-file", help="Protein file that meets specific requirements", type=click.Path(exists=True, dir_okay=False, path_type=Path))
@click.option("--fdr-threshold", help="FDR threshold to filter results", default="0.05", type=float)
...
@click.option("--output-prefix-file", help="Prefix for output files")
@click.option("--delete-existing", help="Delete existing files in output folder", is_flag=True)
