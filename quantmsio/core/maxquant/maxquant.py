...
class MaxQuant:
    ...
    def iter_protein_groups_batch(
        self, protein_groups_path: str, chunksize: int = 1000000, protein_str=None
    ):
        """Iterate over MaxQuant proteinGroups.txt file in batches."""
        # Read header to extract dynamic column names
        header_df = pd.read_csv(protein_groups_path, sep="\t", nrows=0)

        # Find intensity columns for different samples
        ...
        for col in header_df.columns:
            if (
                col.startswith("Intensity ")
                and not col.startswith("Intensity L")
                and not col.startswith("Intensity H")
            ):
                sample_intensity_cols.append(col)
            elif col.startswith("LFQ intensity "):
                sample_lfq_cols.append(col)
            elif col.startswith("iBAQ "):
                sample_ibaq_cols.append(col)

        # Create dynamic column mapping
        use_cols = MAXQUANT_PG_USECOLS.copy()
        use_cols.extend(sample_intensity_cols)
        use_cols.extend(sample_lfq_cols)
        use_cols.extend(sample_ibaq_cols)

        # Filter columns that actually exist in the file
        existing_cols = [col for col in use_cols if col in header_df.columns]
        ...
        for chunk in pd.read_csv(
            ...
            if protein_str:
                chunk = chunk[chunk["Protein IDs"].str.contains(protein_str, na=False)]

            # Store dynamic columns for later use
            chunk.attrs["sample_intensity_cols"] = sample_intensity_cols
            chunk.attrs["sample_lfq_cols"] = sample_lfq_cols
            chunk.attrs["sample_ibaq_cols"] = sample_ibaq_cols

            yield chunk

    def transform_protein_groups(self, df: pd.DataFrame):
        """Transform MaxQuant protein groups data to quantms.io format."""
        # Rename columns using the mapping
        df.rename(columns=MAXQUANT_PG_MAP, inplace=True)

        # Process protein accessions - split by semicolon
        df["pg_accessions"] = df["pg_accessions"].apply(
            lambda x: x.split(";") if pd.notna(x) else []
        )

        # Process protein names - split by semicolon
        df["pg_names"] = df["pg_names"].apply(
            lambda x: x.split(";") if pd.notna(x) else []
        )

        # Process gene names - split by semicolon
        df["gg_accessions"] = df["gg_accessions"].apply(
            lambda x: x.split(";") if pd.notna(x) else []
        )

        # Set anchor protein (first in the group)
        df["anchor_protein"] = df["pg_accessions"].apply(
            lambda x: x[0] if len(x) > 0 else None
        )

        # Handle decoy and contaminant flags
        df["is_decoy"] = (
            df["is_decoy"].apply(lambda x: 1 if x == "+" else 0).fillna(0).astype(int)
        )
        ...
        # Create peptide count structure
        df["peptides"] = df.apply(
            lambda row: (
                [
                    {
                        "protein_name": protein,
                        "peptide_count": row.get("peptide_count_razor_unique", 0) or 0,
                    }
                    for protein in row["pg_accessions"]
                ]
                if row["pg_accessions"]
                else []
            ),
            axis=1,
        )

        # Create intensities array from sample-specific columns
        sample_intensity_cols = df.attrs.get("sample_intensity_cols", [])
        df["intensities"] = df.apply(
            lambda row: self._create_intensities_from_samples(
                row, sample_intensity_cols
            ),
            axis=1,
        )

        # Create additional intensities for LFQ and iBAQ
        sample_lfq_cols = df.attrs.get("sample_lfq_cols", [])
        sample_ibaq_cols = df.attrs.get("sample_ibaq_cols", [])
        df["additional_intensities"] = df.apply(
            lambda row: self._create_additional_intensities(
                row, sample_lfq_cols, sample_ibaq_cols
            ),
            axis=1,
        )
        ...
        df["additional_scores"] = df.apply(
            lambda row: self._create_additional_scores(row), axis=1
        )

        # Add missing required fields
        df["reference_file_name"] = "combined"  # MaxQuant combines runs
        ...
        cols_to_drop = [
            ...
            if col.startswith(
                ("peptide_count_", "lfq_intensity", "ibaq_intensity", "intensity")
            )
        ]
        df.drop(columns=cols_to_drop, inplace=True, errors="ignore")

    def _create_intensities_from_samples(self, row, sample_intensity_cols):
        """Create intensities array from sample-specific intensity columns."""
        intensities = []

        for col in sample_intensity_cols:
            if col in row.index and pd.notna(row[col]) and row[col] > 0:
                # Extract sample name from column name (e.g., "Intensity Sample1" -> "Sample1")
                sample_name = col.replace("Intensity ", "")
                intensities.append(
                    {
                        "sample_accession": sample_name,
                        "channel": "LFQ",  # Default to LFQ for MaxQuant
                        "intensity": float(row[col]),
                    }
                )

        # If no sample-specific columns, use main intensity
        if (
            not intensities
            and "intensity" in row.index
            and pd.notna(row["intensity"])
            and row["intensity"] > 0
        ):
            intensities.append(
                {
                    "sample_accession": "combined",
                    "channel": "LFQ",
    ...
    def _create_additional_intensities(self, row, sample_lfq_cols, sample_ibaq_cols):
        """Create additional intensities array for LFQ and iBAQ values."""
        additional_intensities = []
        sample_intensity_map = {}

        # Group LFQ intensities by sample
        for col in sample_lfq_cols:
            if col in row.index and pd.notna(row[col]) and row[col] > 0:
                sample_name = col.replace("LFQ intensity ", "")
                key = (sample_name, "LFQ")
                if key not in sample_intensity_map:
                    sample_intensity_map[key] = []
                sample_intensity_map[key].append(
                    {
                        "intensity_name": "lfq",
        ...
        # Group iBAQ intensities by sample
        for col in sample_ibaq_cols:
            if col in row.index and pd.notna(row[col]) and row[col] > 0:
                sample_name = col.replace("iBAQ ", "")
                key = (sample_name, "LFQ")
                if key not in sample_intensity_map:
                    sample_intensity_map[key] = []
                sample_intensity_map[key].append(
        ...
        for (sample_name, channel), intensities in sample_intensity_map.items():
    ...
    def _create_additional_scores(self, row):
        """Create additional scores from MaxQuant-specific metrics."""
        scores = []

        score_mapping = {
            "andromeda_score": "andromeda_score",
            "sequence_coverage": "sequence_coverage_percent",
            "molecular_weight": "molecular_weight_kda",
            "msms_count": "msms_count",
        }

        for col, score_name in score_mapping.items():
            if col in row.index and pd.notna(row[col]):
                scores.append(
                    {"score_name": score_name, "score_value": float(row[col])}
                )
