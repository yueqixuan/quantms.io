...
class DuckDBEngine:
    ...
    def __init__(
        ...
        self._conn.execute(f"SET memory_limit='{_validate_set_value(memory_limit, 'memory_limit')}'")
        self._conn.execute(f"SET threads={int(threads)}")
        if temp_directory:
            self._conn.execute(f"SET temp_directory='{_validate_set_value(temp_directory, 'temp_directory')}'")
    ...
    def _setup_s3(self, config: dict) -> None:
        ...
        if "region" in config:
            self._conn.execute(f"SET s3_region='{_validate_set_value(config['region'], 's3_region')}'")
        if "access_key_id" in config and "secret_access_key" in config:
            self._conn.execute(f"SET s3_access_key_id='{_validate_set_value(config['access_key_id'], 's3_access_key_id')}'")
            self._conn.execute(
                f"SET s3_secret_access_key='{_validate_set_value(config['secret_access_key'], 's3_secret_access_key')}'"
            )
        if "endpoint" in config:
            self._conn.execute(f"SET s3_endpoint='{_validate_set_value(config['endpoint'], 's3_endpoint')}'")
    ...
    def register_parquet(self, name: str, file_path: str | Path) -> None:
        ...
        self._conn.execute(f"CREATE OR REPLACE VIEW {name} AS SELECT * FROM read_parquet('{file_path}')")
    ...
    def register_partitioned_parquet(self, name: str, directory: str | Path) -> None:
        ...
        self._conn.execute(
            f"CREATE OR REPLACE VIEW {name} AS SELECT * FROM read_parquet('{glob_pattern}', hive_partitioning=true)"
        )

    def register_s3_parquet(self, name: str, s3_path: str) -> None:
        """Register an S3 Parquet file as a DuckDB view."""
        self._conn.execute(f"CREATE OR REPLACE VIEW {name} AS SELECT * FROM read_parquet('{s3_path}')")
...
def create_converter_connection(
    ...
    conn.execute(f"SET memory_limit='{_validate_set_value(memory_limit, 'memory_limit')}'")
