"""
This file contains the classes to manage and handler the QPX differential expression files. The differential
...
The QPX differential expression file format is described in the docs folder of this repository.
(https://github.com/bigbio/QPX/blob/main/docs/DE.md)
"""
...
from qpx.utils.file_utils import delete_files_extension
...
#INFO=<ID=issue, Number=1, Type=String, Description="Issue column shows if there is any issue for inference in corresponding protein and comparison">\n"""
    ...
    def load_project_file(self, project_file: Union[Path, str]):
        ...
        https://github.com/bigbio/QPX/blob/main/docs/PROJECT.md
    ...
    def convert_msstats_to_quantms(
        ...
        """
        Convert a MSstats differential file to QPX format
        :return: none
        """
        ...
        if self.project_manager:
            ...
            output_lines += (
                "#qpx_version: "
                + self.project_manager.project.project_info["qpx_version"]
                + "\n"
            )
        else:
