...
def test_transform_protein_groups():
    ...
    from quantmsio.core.diann import DIANN_PG_SQL
    ...
    if len(report) > 0:  # Only test if we have data
        ...
        if len(df) > 0:
            ...
            assert "intensities" in pg_df.columns, "intensities column should be present"
            assert "additional_intensities" in pg_df.columns, "additional_intensities column should be present"
            ...
            assert "sample_accession" in intensity_entry, "intensity entry should have sample_accession"
            assert "channel" in intensity_entry, "intensity entry should have channel"
            assert "intensity" in intensity_entry, "intensity entry should have intensity value"
            assert intensity_entry["channel"] == "LFQ", "channel should be LFQ for DIA-NN"
            
            # Check additional_intensities structure
            additional_intensity_sample = pg_df["additional_intensities"].iloc[0]
            assert isinstance(additional_intensity_sample, list), "additional_intensities should be a list"
            assert len(additional_intensity_sample) > 0, "additional_intensities should not be empty"
            
            additional_entry = additional_intensity_sample[0]
            assert "sample_accession" in additional_entry, "additional_intensity entry should have sample_accession"
            assert "channel" in additional_entry, "additional_intensity entry should have channel"
            assert "additional_intensity" in additional_entry, "additional_intensity entry should have additional_intensity array"
            assert additional_entry["channel"] == "LFQ", "channel should be LFQ for DIA-NN"
            
            # Check the additional intensity array structure
            additional_types = additional_entry["additional_intensity"]
            assert isinstance(additional_types, list), "additional_intensity should be a list"
            assert len(additional_types) == 2, "should have 2 additional intensity types (normalize_intensity and lfq)"
            
            # Check normalize_intensity and lfq are present
            intensity_names = [item["intensity_name"] for item in additional_types]
            assert "normalize_intensity" in intensity_names, "normalize_intensity should be present"
