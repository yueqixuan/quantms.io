...
@pytest.mark.parametrize("test_name, backend, mztab_path, msstats_path", TEST_CASES)
def test_create_duckdb_and_parquet(test_name, backend, mztab_path, msstats_path):
    """
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        if backend == "duckdb":
            ...
            output_path = Path(temp_dir) / f"test_{test_name}"
        ...
        print(
            f"\nTime to process '{test_name}' with '{backend}' backend: {elapsed_time:.2f} seconds"
        )

        # Verification logic depends on the backend used
        con = None
        try:
            if backend == "duckdb":
