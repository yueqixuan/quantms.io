...
def test_extract_ms_runs():
    ...
    for backend in backends:
        ...
        try:
            ...
            indexer = MzTabIndexer.create(
                mztab_path=MZTAB_TEST_PATH, database_path=temp_db_path, backend=backend
            )
            ...
            if backend == "duckdb":
                ...
                # For parquet backend, remove the directory and all contents
                if os.path.exists(temp_db_path):
                    import shutil
...
def test_ms_run_extraction_both_backends():
    """Test MS run extraction utility functions with both backends."""
    ...
    for backend in backends:
        logger.info(f"Testing MS run extraction with {backend} backend")
        ...
        try:
            ...
            logger.info(
                f"✅ {backend} backend utility function test passed successfully"
            )
...
def test_get_protein_qvalue():
    ...
    for backend in backends:
        logger.info(f"Testing protein q-value extraction with {backend} backend")
        ...
        try:
            ...
            if protein_count > 0:
                ...
                logger.info(
                    f"First 10 proteins with lowest q-values ({backend} backend):"
                )
                ...
                # Additional analysis
                qvalues = [
                    float(q) for q in protein_qvalue_map.values() if q is not None
                ]
                if qvalues:
                    ...
                    logger.info(
                        f"  Proteins with q-values: {len(qvalues)}/{protein_count}"
                    )
                ...
                if protein_groups:
                    ...
                    logger.info(
                        f"{'Accession':<50} {'Protein Count':<15} {'Q-value':<10}"
                    )
                    ...
                    for i, (accession, qvalue) in enumerate(sorted_groups[:10], 1):
                        ...
                        logger.info(
                            f"{accession:<50} {protein_count:<15} {qvalue_str:<10}"
                        )
                else:
            ...
            logger.info(
                f"✅ {backend} backend protein q-value test passed successfully"
            )
...
def test_get_metadata_modifications():
    ...
    try:
        ...
        indexer = MzTabIndexer.create(
            ...
            backend="duckdb",  # Using duckdb backend for simplicity
        )
