...
def test_fragment_annotation_fields_types():
    ...
    assert charge_field.type == pa.list_(pa.int32()), "charge_array should be list of int32"
    ...
    assert ion_type_field.type == pa.list_(pa.string()), "ion_type_array should be list of string"
    ...
    assert ion_mobility_field.type == pa.list_(pa.float32()), "ion_mobility_array should be list of float32"
...
def test_fragment_annotation_fields_have_descriptions():
    ...
    for field_name in ['charge_array', 'ion_type_array', 'ion_mobility_array']:
        ...
        assert b'description' in field.metadata, f"{field_name} should have description in metadata"
        description = field.metadata[b'description'].decode()
        assert len(description) > 0, f"{field_name} should have non-empty description"
        assert 'fragment' in description.lower(), f"{field_name} description should mention 'fragment'"
