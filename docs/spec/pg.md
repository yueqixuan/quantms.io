...
!!! warning "Primary key constraints"
    The combination of `anchor_protein` and `grouped_runs` forms the composite primary key. Both fields MUST NOT be null. `grouped_runs` is a **set of distinct raw files**: it MUST NOT contain duplicates, and two keys are equal when they contain the same files regardless of order (identity is compared set-wise). The list is **stored in fraction order** — sorting is not applied, because it would destroy fraction ordering (and lexicographically misorder names like `F1, F10, F2`). Each record represents a single protein group quantified in one quantification unit (the group of raw files/fractions aggregated together). A protein quantity only exists after aggregating peptides across a sample's fractions, so the PG view keys on this group of raw files, not a single raw file; for unfractionated or DIA data the list has a single element.
