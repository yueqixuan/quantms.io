...
QPX field names are mapped to [PSI-MS ontology](https://www.ebi.ac.uk/ols4/ontologies/ms) terms where applicable. This mapping provides formal semantic definitions for each field, enabling interoperability with other proteomics tools and standards.

!!! info "Fields without ontology mappings"
    Fields with blank ontology entries in the table below are QPX-specific fields without standardized MS vocabulary terms. These fields are defined entirely by the QPX specification.

## Mapping Table

| Field | Ontology Name | Ontology Accession | Description |
|---|---|---|---|
| `sequence` | AA sequence | `MS:1001344` | The sequence is an amino acid sequence. |
...
| `number_peaks` | number of peaks | `MS:1003059` | Number of peaks or features in a spectrum. For a peak-picked spectrum, this will correspond to the number of data points. For a non-peak-picked spectrum, this corresponds to the number of features discernable in the spectrum, which will be fewer than the number of data points. |
| `mz_array` | m/z array | `MS:1000514` | A data array of m/z values. |
...
The following fields represent search engine and post-processing scores that also have PSI-MS ontology mappings.

| Field | Ontology Name | Ontology Accession | Description |
|---|---|---|---|
| `andromeda_score` | Andromeda:score | `MS:1002338` | The probability based score of the Andromeda search engine. |
| `andromeda_delta_score` | Andromeda:delta score | `MS:1003433` | Score difference to the second best identified peptide with a different amino acid sequence. |
...
The following fields are defined by QPX and do not have standardized PSI-MS ontology terms.
...
| `precursor_quantification_score` | When using QuantUMS, equal to 1.0 / (1.0 + SD), where SD is the standard deviation of the LC-MS-derived error in relative precursor quantification. |

!!! note "Ontology references"
    All ontology accessions refer to the [PSI-MS controlled vocabulary](https://www.ebi.ac.uk/ols4/ontologies/ms). You can look up any term by searching for its accession (e.g., `MS:1000894`) in the [OLS4 browser](https://www.ebi.ac.uk/ols4/).
