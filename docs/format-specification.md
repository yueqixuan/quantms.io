...
| _view_       | _file class_      | _serialization format_ | _definition_                  |
| ------------ | ----------------- | ---------------------- | ----------------------------- |
| mz           | mz_file           | _parquet_              | [mz](#mz)                     |
| psm          | psm_file          | _parquet_              | [psm](#psm)                   |
| feature      | feature_file      | _parquet_              | [feature](#feature)           |
| pg           | pg_file           | _parquet_              | [pg](#pg)                     |
| peptide      | peptide_file      | _parquet_              | [peptide](#peptide)           |
| protein      | protein_file      | _parquet_              | [protein](#protein)           |
...
The `scan` is use in the following section: [psm](#psm), [feature](#feature), [mz](#mz).
...
When registering parquet files to project.json [project](#project), it will be in such a format.
...
The files within a project are in the current version [version](#version) optional. Files within a project should be listed in the quantms_files, for every file the following information is necessary:
...
| _protein_  | _label_                          | _log2fc_ | _se_ | _df_ | _pvalue_ | _adj_pvalue_ | _issue_ |
| ---------- | -------------------------------- | -------- | ---- | ---- | -------- | ------------ | ------- |
...
- [feature](#feature): Peptide Feature View—The peptide feature views (peptide features) aims to cover detail on quantified peptide information level, including peptide intensity in relation to the sample metadata.
...
The feature file is similar to the [mztab](https://https://github.com/HUPO-PSI/mzTab/tree/master/specification_document-releases/1_0-Proteomics-Release) peptide table, the peptide evidence in [MaxQuant](https://cox-labs.github.io/coxdocs/output_tables.html), the [diann matrix table](https://github.com/vdemichev/DiaNN/blob/master/README.md#output).
...
- Different to the psm section [psm](#psm) contains all the protein inference information depending on if protein inference was applied or not.
...
| `pg_names`               | Protein group names                                                                                                                                                              | array[string], null         | Protein.Names            | -                                  | Protein names                                                |
| `gg_accessions`          | Gene group accessions, as a string array                                                                                                                                         | array[string], null         | Genes                    | -                                  | Gene names                                                   |
| `gg_names`               | Gene names corresponding to the proteins in the group                                                                                                                            | array[string], null         | -                        | -                                  | -                                                            |
| `reference_file_name`    | The raw file containing the identified/quantified protein                                                                                                                        | string                      | Run                      | -                                  | combined                                                     |
| `peptide_counts`         | Peptide sequence counts for this protein group in this specific file. Contains unique sequences (specific to this protein group) and total sequences.                            | struct, null                | Unique.Stripped.Peptides | Unique Peptides                    | Unique peptides                                              |
| `feature_counts`         | Peptide feature counts (peptide charge combinations) for this protein group in this specific file. Contains unique features (specific to this protein group) and total features. | struct, null                | Precursor.Quantity       | Precursor Ions                     | MS/MS count                                                  |
| `global_qvalue`          | Global q-value of the protein group at the experiment level                                                                                                                      | float32, null               | Global.PG.Q.Value        | -                                  | Q-value                                                      |
| `pg_qvalue`              | Protein group q-value at the run level (DIA-NN specific)                                                                                                                         | float32, null               | PG.Q.Value               | -                                  | -                                                            |
| `intensities`            | The primary intensity-based abundance of the protein group in the sample across different channels. Contains raw/primary measurements from the quantification tool.              | [intensities](#intensities), null | PG.Quantity              | -                                  | Intensity, LFQ intensity                                     |
| `additional_intensities` | Derived/processed intensity values calculated from primary intensities using different algorithms (normalization, LFQ, iBAQ, etc.).                                              | [intensities](#intensities), null | PG.Normalised, PG.MaxLFQ | -                                  | LFQ intensity, iBAQ                                          |
| `peptides`               | Number of peptides per protein in the protein group                                                                                                                              | array[struct]               | -                        | -                                  | Razor + unique peptides, Unique peptides                     |
| `anchor_protein`         | Representative protein of the protein group (usually the first)                                                                                                                  | string, null                | -                        | -                                  | First protein in Protein IDs                                 |
| `is_decoy`               | Decoy indicator                                                                                                                                                                  | int32                       | -                        | -                                  | Reverse                                                      |
| `contaminant`            | Contaminant indicator                                                                                                                                                            | int32, null                 | -                        | -                                  | Potential contaminant                                        |
| `sequence_coverage`      | Percentage of the protein sequence covered by identified peptides                                                                                                                | float32, null               | -                        | -                                  | Sequence coverage [%]                                        |
| `molecular_weight`       | Molecular weight of the protein in kDa                                                                                                                                           | float32, null               | -                        | -                                  | Mol. weight [kDa]                                            |
| `additional_scores`      | Additional scores and metrics                                                                                                                                                    | array[struct], null         | -                        | -                                  | Score, MS/MS count                                           |
| `cv_params`              | Optional list of CV parameters for additional metadata                                                                                                                           | array[struct], null         | -                        | -                                  | -                                                            |
...
| `gg_accessions`        | Gene group accessions                 | null, array[string]                                                                                                                    |
| `gg_names`             | Gene group names                      | null, array[string]                                                                                                                    |
...
> **Note 1**: score_struct is a record with fields: `score_name` (string) and `score_value` (float32). See [protein.avsc](protein.avsc) for the full AVSC definition.
...
| `inverse_ion_mobility` | float, null                                      | Inverse ion mobility, if available, used for TIMS data                                                                                                                                                                        |
...
| `precursors`           | array[struct], null                              | List of precursors for this scan, if applicable. Each precursor contains: selected_ion_mz, selected_ion_charge, selected_ion_intensity, isolation_window_target, isolation_window_lower, isolation_window_upper, spectrum_ref |
| `mz`                   | array[float]                                     | List of m/z values for the scan                                                                                                                                                                                               |
